...
txscripts ::
  ...
  Core.Tx era ->
  Set (Core.Script era)
txscripts (UTxO m) tx = witnessScripts `Set.union` referenceScripts
  where
    txbody = getField @"body" tx
    allInputs = Set.union (inputs' txbody) (referenceInputs' txbody)
...
languages utxo tx =
  Set.fromList
    [ lang
      | script <- Set.toList (txscripts utxo tx),
